...
# ----- post-ingest meditation (extraction is staged + ingested separately) --


def meditate_all_stores(cfg, store, summarizer):
    """Run meditation over the three bounded stores when they overflow.

    The post-ingest half of one persona's sweep (design §5): for each store,
    meditate ONLY when it is at/above its overflow limit (the hysteresis
    trigger — :meth:`BoundedStore.is_over_overflow`); meditation itself is a
    no-op under ``max``. ``mission_text`` for the must_remember relevance-check
    is sourced from the charter (Miyagi owns the sourcing, plan §1) and threaded
    into :func:`meditation.meditate`; the relevance/downgrade *logic* lives in
    ``meditate``. A live per-store lock held by another session is skipped (not
    urgent — retried next sweep).

    Returns a ``store_name -> MeditationLog`` map for the stores actually
    meditated. Pure orchestration — the judgement is the summarizer's.
    """
    from .bounded_store import BoundedStore, StoreLockHeld
    from .entries import STORE_NAMES
    from .lifecycle import team_mission_text
    from .meditation import meditate

    bstore = BoundedStore(cfg, store)
    mission_text = team_mission_text(cfg)
    logs: dict = {}
    for store_name in STORE_NAMES:
        entries = bstore.load(store_name)
        if not bstore.is_over_overflow(store_name, entries):
            continue
        log.info("sweep: %s over overflow; meditating", store_name)
        try:
            logs[store_name] = meditate(
                store_name, cfg.agent.name, mission_text, summarizer, cfg, bstore
            )
        except StoreLockHeld:
            log.info("sweep: %s locked by another session; skipping", store_name)
    return logs
