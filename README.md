...
| `tigerharness.autodrive` | **Periodic journal driver** (the Operator-authorized exception to the human-only drive rule). A detached daemon runs "drive the journal" on a fixed interval via the backend-agnostic `agent_sdk` (default `claude -p`) — drive, then sleep, no overlap; not built on `/loop`. Only safe while `claude -p` bills the subscription, so it ships with a `--max-budget` cap, a 60s interval floor, single-instance, and an `autodrive stop` off-switch. See [docs/autodrive.md](docs/autodrive.md). |
